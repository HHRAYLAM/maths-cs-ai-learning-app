...
- 两个任务共享相同的底层表示：固定维度的说话人嵌入，捕捉说话人的身份，无论他们说什么。区别仅在于决策阶段：验证比较两个嵌入，识别在候选者中找到最近的嵌入。

- 余弦相似度 是比较说话人嵌入的标准度量。给定注册嵌入 $e$ 和测试嵌入 $t$：

$$s = \frac{e \cdot t}{\|e\| \, \|t\|}$$

- 阈值 $\theta$ 决定接受/拒绝决策：如果 $s > \theta$，接受。阈值在错误接受率（FAR）和错误拒绝率（FRR）之间权衡。等错误率（EER）（FAR = FRR 处）是标准评估指标。更低的 EER 意味着更好的性能。最先进的系统在标准基准（VoxCeleb）上实现低于 1% 的 EER。

- i-vector（Dehak 等人，2010）是深度学习之前主导的说话人嵌入。这个想法来自因子分析（第 02 章的矩阵分解和第 04 章的降维）。通用背景模型（UBM）（在多样化说话人上训练的大型 GMM）定义超向量空间。每个话语的 GMM 超向量被投影到低维总变异性空间：
...
- x-vector（Snyder 等人，2018）使用时延神经网络（TDNN） 架构显著推进了神经说话人嵌入。TDNN 是具有每层特定上下文窗口的一维卷积，与文件 03 的 WaveNet 中的空洞卷积相关，但应用于帧级特征而非原始波形样本。
...
- 节拍跟踪 检测音乐的节奏脉冲。标准方法从谱图计算起音强度包络（检测信号音符起音的能量增加），然后使用自相关或 tempo 图找到 tempo，最后使用动态规划跟踪单个节拍位置，找到最佳匹配起音包络同时保持一致 tempo 的节拍时间序列。
