...
- Vulkan 冗长 — "hello world"计算程序约 300 行 C++。但这种冗长是**显式控制**的代价：你管理每个 GPU 资源（内存、管道、命令缓冲区）。这种控制实现最大性能和可移植性，代价是开发速度。
...
- **计算着色器**是在 GPU 上运行的程序，类似于 CUDA 内核。它用**GLSL**（OpenGL 着色语言）编写，编译为**SPIR-V**字节码（可移植二进制格式）。
